...
public class HodaGi_C_Bot extends Base_HodaGi_Bot {
	...
	public static final String ID = "block_hodagi_c_bot";
		...
		if (i != 1) {
			if (itemstack.isEmpty() && hitY < 0.75F) {
				...
				if (i == 3) {
					worldIn.setBlockState(pos, Crop_Blocks.HODAGI_A_BOT.getDefaultState()
							.withProperty(H_FACING, state.getValue(H_FACING))
							.withProperty(HodaGi_A_Bot.STAGE_1_4, Integer.valueOf(2))); }

				if (i == 4) {
					worldIn.setBlockState(pos, Crop_Blocks.HODAGI_A_BOT.getDefaultState()
							.withProperty(H_FACING, state.getValue(H_FACING))
							.withProperty(HodaGi_A_Bot.STAGE_1_4, Integer.valueOf(3))); } }
