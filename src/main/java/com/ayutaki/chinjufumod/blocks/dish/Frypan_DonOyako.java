...
public class Frypan_DonOyako extends BaseFrypan_Stage2 {

	public Frypan_DonOyako(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 1) {
			if (item == Items_Teatime.DONBURI_MESHI) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.DONBURI_OYAKO)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.DONBURI_OYAKO))) {
					playerIn.drop(new ItemStack(Items_Teatime.DONBURI_OYAKO), false); }
