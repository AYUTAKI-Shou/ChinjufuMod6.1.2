...
public class Tamagoyaki extends BaseFood_Stage3Water {
	...
	public Tamagoyaki(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 3) {
			...
			if (itemstack.isEmpty()) {
				...
				if (i == 1) {
					playerIn.addEffect(new EffectInstance(Effects.REGENERATION, 80, 0));
					playerIn.addEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
	
				if (i == 2) {
					playerIn.addEffect(new EffectInstance(Effects.REGENERATION, 160, 0));
					playerIn.addEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
	...
	public ItemStack getCloneItemStack(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.getValue(STAGE_1_3);
		return (i == 1)? new ItemStack(Items_Teatime.TAMAGOYAKI) : new ItemStack(Items_Teatime.SARA);
	}
