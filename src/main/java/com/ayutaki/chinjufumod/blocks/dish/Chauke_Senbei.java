...
public class Chauke_Senbei extends Block implements IWaterLoggable {
	...
	/* Collision */
	protected static final VoxelShape AABB_BOX = Block.box(4.5D, 0.0D, 4.5D, 11.5D, 1.5D, 11.5D);
	protected static final VoxelShape AABB_DOWN = Block.box(4.5D, -8.0D, 4.5D, 11.5D, 0.1D, 11.5D);

	public Chauke_Senbei(AbstractBlock.Properties properties) {
		super(properties);
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 5) {
			...
			if (itemstack.isEmpty()) {
				...
				playerIn.inventory.add(new ItemStack(Items_Teatime.SENBEI));
	...
	public ItemStack getCloneItemStack(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.getValue(STAGE_1_5);
		return (i == 1)? new ItemStack(Items_Teatime.CHAUKE_SENBEI) : new ItemStack(Items_Teatime.SARA);
	}
