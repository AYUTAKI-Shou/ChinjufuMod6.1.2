...
public class Tonsui_small extends BaseStage3_FaceDown {

	public static final String ID = "block_food_tonsui_1";
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (i != 3) {
			...
			if (itemstack.isEmpty()) {
				...
				if (i == 2) {
					/** リジェネは1800 即時回復は0,0でよい 満腹は2で肉メモリの1個分 **/
					((EntityLivingBase) playerIn).addPotionEffect(new PotionEffect(MobEffects.SATURATION, 1, 0));
					((EntityLivingBase) playerIn).addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 1800, 0)); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		if (i == 1) { stack.add(new ItemStack(Items_Teatime.TONSUITORI, 1, 0)); }
		if (i != 1) { stack.add(new ItemStack(Items_Teatime.Item_DISH, 1, 5)); }
		return stack;
	}

	@Override
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World worldIn, BlockPos pos, EntityPlayer playerIn) {
		return new ItemStack(Items_Teatime.TONSUITORI, 1, 0);
	}
