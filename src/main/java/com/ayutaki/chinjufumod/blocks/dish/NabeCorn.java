...
public class NabeCorn extends BaseNabe {
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (item == Items_Teatime.SARA) {
			...
			if (itemstack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.CORNSOUP)); }
	...
	public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.get(STAGE_1_4);
		return (i == 1)? new ItemStack(Items_Teatime.NABECORN) : new ItemStack(Items_Teatime.NABE_kara);
	}
