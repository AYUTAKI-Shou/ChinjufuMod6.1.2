...
public class Frypan_PastaTomato extends BaseFrypan_Stage2 {

	public Frypan_PastaTomato(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 1) {
			if (item == Items_Teatime.PASTA_sara) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.PASTATOMATO)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.PASTATOMATO))) {
					playerIn.drop(new ItemStack(Items_Teatime.PASTATOMATO), false); }
