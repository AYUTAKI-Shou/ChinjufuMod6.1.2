...
public class Frypan_PastaKinoko extends BaseFrypan_Stage2 {

	public Frypan_PastaKinoko(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 1) {
			if (item == Items_Teatime.PASTA_sara) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.PASTAKINOKO)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.PASTAKINOKO))) {
					playerIn.drop(new ItemStack(Items_Teatime.PASTAKINOKO), false); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.block_food_frypan_pasta")).withStyle(TextFormatting.GRAY));
	}
