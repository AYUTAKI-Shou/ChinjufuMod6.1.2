...
public class JPTeaKyusu extends BaseFood_Stage5Water {
	...
	public JPTeaKyusu(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 5) {
			if (item == Items_Teatime.YUNOMI) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.JPTEACUP)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.JPTEACUP))) {
					playerIn.drop(new ItemStack(Items_Teatime.JPTEACUP), false); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.block_food_kyusu_1")).withStyle(TextFormatting.GRAY));
	}
