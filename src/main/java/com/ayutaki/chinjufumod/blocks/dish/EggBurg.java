...
public class EggBurg extends BaseFood_Stage3Water {
	...
	public EggBurg(Block.Properties properties) {
		super(properties);
	}
		...
		if (i != 3) {
			...
			if (itemstack.isEmpty()) {
				...
				if (i == 1) {
					...
					playerIn.addPotionEffect(new EffectInstance(Effects.REGENERATION, 150, 0));
					playerIn.addPotionEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
	
				if (i == 2) {
					playerIn.addPotionEffect(new EffectInstance(Effects.REGENERATION, 210, 0));
					playerIn.addPotionEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
	...
	public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.get(STAGE_1_3);
		return (i == 1)? new ItemStack(Items_Teatime.EGGBURG) : new ItemStack(Items_Teatime.SARA);
	}
