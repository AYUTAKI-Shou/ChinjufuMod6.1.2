...
public class EggBurg extends BaseFood_Stage3Water {
	...
	public EggBurg(AbstractBlock.Properties properties) {
		super(properties);
	}
		...
		if (i != 3) {
			...
			if (itemstack.isEmpty()) {
				...
				if (i == 1) {
					...
					playerIn.addEffect(new EffectInstance(Effects.REGENERATION, 150, 0));
					playerIn.addEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
	
				if (i == 2) {
					playerIn.addEffect(new EffectInstance(Effects.REGENERATION, 210, 0));
					playerIn.addEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
