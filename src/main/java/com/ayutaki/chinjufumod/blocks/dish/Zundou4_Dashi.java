...
public class Zundou4_Dashi extends BaseZundou_4Stage {
	...
	public Zundou4_Dashi(Block.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (item == Items_Teatime.SHOUYU_donburi) {
			...
			if (itemstack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.TSUYU_donburi)); }
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.TSUYU_donburi))) {
				playerIn.dropItem(new ItemStack(Items_Teatime.TSUYU_donburi), false); }
	...
	public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.get(STAGE_1_4);
		return (i == 1)? new ItemStack(Items_Teatime.ZUNDOU_DASHI) : new ItemStack(Items_Teatime.ZUNDOU);
	}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.block_food_dashinabe_1")).applyTextStyle(TextFormatting.GRAY));
	}
