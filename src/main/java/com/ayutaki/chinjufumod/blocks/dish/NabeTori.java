...
public class NabeTori extends BaseNabe {
	...
	public NabeTori(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (item == Items_Teatime.TONSUI) {
			...
			if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.TONSUITORI)); }
			else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.TONSUITORI))) {
				playerIn.drop(new ItemStack(Items_Teatime.TONSUITORI), false); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.block_food_nabe_1")).withStyle(TextFormatting.GRAY));
	}
