...
public class Pizza extends Block implements IWaterLoggable {
	...
	protected static final VoxelShape AABB6_BOX = Block.box(1.0D, 0.0D, 1.0D, 15.0D, 2.5D, 15.0D);
	protected static final VoxelShape AABB6_DOWN = Block.box(1.0D, -8.0D, 1.0D, 15.0D, 0.1D, 15.0D);

	public Pizza(AbstractBlock.Properties properties) {
		super(properties);
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i < 5) {
			...
			if (itemstack.isEmpty()) {
				...
				playerIn.inventory.add(new ItemStack(Items_Teatime.PC_PIZZA));
	...
	public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		...
		if (i < 5) {
			if (inWater(state, worldIn, pos)) {
				...
				worldIn.setBlock(pos, state.setValue(STAGE_1_6, Integer.valueOf(5)), 3);
