...
public class Pizza extends Block implements IWaterLoggable {
	...
	protected static final VoxelShape AABB6_BOX = Block.makeCuboidShape(1.0D, 0.0D, 1.0D, 15.0D, 2.5D, 15.0D);
	protected static final VoxelShape AABB6_DOWN = Block.makeCuboidShape(1.0D, -8.0D, 1.0D, 15.0D, 0.1D, 15.0D);

	public Pizza(Block.Properties properties) {
		super(properties);
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i < 5) {
			...
			if (itemstack.isEmpty()) {
				...
				playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.PC_PIZZA));
	...
	public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
		...
		if (i == 6) { return flag? AABB6_BOX : AABB6_DOWN; }
	...
	public ItemStack getItem(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.get(STAGE_1_6);
		return (i == 1)? new ItemStack(Items_Teatime.PIZZA) : new ItemStack(Items.AIR);
	}
	...
	public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		...
		if (i < 5) {
			if (inWater(state, worldIn, pos)) {
				...
				worldIn.setBlockState(pos, state.with(STAGE_1_6, Integer.valueOf(5)));
