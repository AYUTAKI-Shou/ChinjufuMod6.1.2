...
public class Bottle_SakeJuku extends Base_Bottle {
	...
	public Bottle_SakeJuku(Block.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 5) {
			if (item == Items_Teatime.DRINKGLASS) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.JUKUSAKEGLASS)); }
				else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.JUKUSAKEGLASS))) {
					playerIn.dropItem(new ItemStack(Items_Teatime.JUKUSAKEGLASS), false); }
