...
public class Bottle_WineJuku extends Base_Bottle {
	...
	public Bottle_WineJuku(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 5) {
			if (item == Items_Teatime.DRINKGLASS) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.JUKUWINEGLASS)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.JUKUWINEGLASS))) {
					playerIn.drop(new ItemStack(Items_Teatime.JUKUWINEGLASS), false); }
