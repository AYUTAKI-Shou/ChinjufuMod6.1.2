...
public class NabeAmazake extends BaseNabe {
	...
	public NabeAmazake(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (item == Items_Teatime.YUNOMI) {
			...
			if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.AMAZAKEGLASS)); }
			else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.AMAZAKEGLASS))) {
				playerIn.drop(new ItemStack(Items_Teatime.AMAZAKEGLASS), false); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.block_food_nabeaz_1")).withStyle(TextFormatting.GRAY));
	}
