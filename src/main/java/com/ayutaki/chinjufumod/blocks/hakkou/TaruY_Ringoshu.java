...
public class TaruY_Ringoshu extends BaseTaru_Yoh {
	...
	public TaruY_Ringoshu(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i > 3) {
			if (item == Items_Teatime.SAKEBOTTLE) {
				...
				if (i == 4 || i == 5) {
					if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.CIDERBOT)); }
					else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.CIDERBOT))) {
						playerIn.drop(new ItemStack(Items_Teatime.CIDERBOT), false); } }
		
				if (i == 6) {
					if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.JUKUCIDERBOT)); }
					else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.JUKUCIDERBOT))) {
						playerIn.drop(new ItemStack(Items_Teatime.JUKUCIDERBOT), false); } }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.block_taru_ringoshu")).withStyle(TextFormatting.GRAY));
	}
