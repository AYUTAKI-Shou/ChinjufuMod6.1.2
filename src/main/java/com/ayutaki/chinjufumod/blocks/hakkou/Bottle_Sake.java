...
public class Bottle_Sake extends Base_Bottle {
	...
	public Bottle_Sake(AbstractBlock.Properties properties) {
		super(properties);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i != 5) {
			if (item == Items_Teatime.DRINKGLASS) {
				...
				if (itemstack.isEmpty()) { playerIn.inventory.add(new ItemStack(Items_Teatime.SAKEGLASS)); }
				else if (!playerIn.inventory.add(new ItemStack(Items_Teatime.SAKEGLASS))) {
					playerIn.drop(new ItemStack(Items_Teatime.SAKEGLASS), false); }
	...
	public ItemStack getCloneItemStack(IBlockReader worldIn, BlockPos pos, BlockState state) {
		int i = state.getValue(STAGE_1_5);
		return (i == 1)? new ItemStack(Items_Teatime.SAKEBOT) : new ItemStack(Items_Teatime.SAKEBOTTLE);
	}
