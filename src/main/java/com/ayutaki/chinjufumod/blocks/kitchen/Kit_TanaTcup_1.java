...
public class Kit_TanaTcup_1 extends Base_Tana7 {
	...
	public Kit_TanaTcup_1(AbstractBlock.Properties properties) {
		super(properties);
	}
		...
		if (item != Items_Teatime.TCUP_kara && item != Items_Teatime.TEAPOT_kara) {
			if (itemstack.isEmpty()) {
				if (i != 7) {
					...
					playerIn.inventory.add(new ItemStack(Items_Teatime.TCUP_kara));
				...
				if (i == 7) {
					CMEvents.soundItemPick(worldIn, pos);
					playerIn.inventory.add(new ItemStack(Items_Teatime.TEAPOT_kara));
