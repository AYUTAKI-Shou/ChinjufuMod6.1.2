...
public class WorldGenTree_KaedeBig extends WorldGenAbstractTree {
	...
	List<WorldGenTree_KaedeBig.FoliageCoordinates> foliageCoords;
	/* 葉っぱのフィールドを追加 */
	private IBlockState leave;

	public WorldGenTree_KaedeBig(boolean notify) {
		super(notify);
		this.leave = Seasonal_Blocks.KAEDE_leaf.getDefaultState();
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTree_KaedeBig.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTree_KaedeBig.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords.add(new WorldGenTree_KaedeBig.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeafNode(BlockPos pos) {
		...
		for (int i = 0; i < this.leafDistanceLimit; ++i) {
			...
			/** Blocks.LEAVES.getDefaultState()…を、サクラに書き換え **/
	...
	void generateLeaves() {
		...
		for (WorldGenTree_KaedeBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
