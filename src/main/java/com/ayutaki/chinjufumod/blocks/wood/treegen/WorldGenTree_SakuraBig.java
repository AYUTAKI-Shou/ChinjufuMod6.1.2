...
/* WorldGenBigTreeを参照 */
public class WorldGenTree_SakuraBig extends WorldGenAbstractTree {
	...
	List<WorldGenTree_SakuraBig.FoliageCoordinates> foliageCoords;
	/* 葉っぱのフィールドを追加 */
	private IBlockState leave;

	public WorldGenTree_SakuraBig(boolean notify) {
		super(notify);
		this.leave = Seasonal_Blocks.SAKURA_flow.getDefaultState();
	}

	public WorldGenTree_SakuraBig(boolean notify, IBlockState state) {
		super(notify);
	 this.leave = state;
	}
	...
	public Block getLog() {
	 return Seasonal_Blocks.SAKURA_log;
	}

	public Block getSapling() {
	 return Seasonal_Blocks.SAKURA_nae;
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTree_SakuraBig.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTree_SakuraBig.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords.add(new WorldGenTree_SakuraBig.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeafNode(BlockPos pos) {
		...
		for (int i = 0; i < this.leafDistanceLimit; ++i) {
			...
			/** Blocks.LEAVES.getDefaultState()…を、getLeaveState()に書き換え **/
			this.crosSection(pos.up(i), this.leafSize(i), getLeaveState());
		}
	}
	...
	void generateLeaves() {
		...
		for (WorldGenTree_SakuraBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
	...
	void generateLeafNodeBases() {
		...
		for (WorldGenTree_SakuraBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
			...
			if (!blockpos.equals(BlockBigTreeSakura$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY())) {
				/** Blocks.LOG; を getLog(); に書き換え **/
				this.limb(blockpos, BlockBigTreeSakura$foliagecoordinates, getLog());
			}
		}
	}
