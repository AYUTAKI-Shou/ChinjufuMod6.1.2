...
public class TrayFude extends Block implements IWaterLoggable {
	...
	public TrayFude(Block.Properties properties) {
		super(properties);
		...
		if (state.get(LOST) != true) {
			...
			if (item == Items.BOOK) {
				...
				if (playerIn.experienceTotal >= 50) {
					...
					if (Config_CM.getInstance().useMAKIMONO() == true) {
						switch (direction) {
						case NORTH :
						default :
							/** left **/
							if (eaststate.getMaterial().isReplaceable()) {
								...
								worldIn.setBlockState(pos.east(), Unit_Blocks.WRITTEN_MAKIMONO.getDefaultState()
										.with(WrittenMakimono.H_FACING, Direction.NORTH)
										.with(WrittenMakimono.DOWN, state.get(WrittenMakimono.DOWN))
										.with(WrittenMakimono.WATERLOGGED, state.get(WrittenMakimono.WATERLOGGED)), 3); }
						...
						case SOUTH :
							...
							if (weststate.getMaterial().isReplaceable()) {
								this.writebook(worldIn, pos, playerIn, hand);
								worldIn.setBlockState(pos.west(), Unit_Blocks.WRITTEN_MAKIMONO.getDefaultState()
										.with(WrittenMakimono.H_FACING, Direction.SOUTH)
										.with(WrittenMakimono.DOWN, state.get(WrittenMakimono.DOWN))
										.with(WrittenMakimono.WATERLOGGED, state.get(WrittenMakimono.WATERLOGGED)), 3); }
						...
						case EAST :
							...
							if (southstate.getMaterial().isReplaceable()) {
								this.writebook(worldIn, pos, playerIn, hand);
								worldIn.setBlockState(pos.south(), Unit_Blocks.WRITTEN_MAKIMONO.getDefaultState()
										.with(WrittenMakimono.H_FACING, Direction.EAST)
										.with(WrittenMakimono.DOWN, state.get(WrittenMakimono.DOWN))
										.with(WrittenMakimono.WATERLOGGED, state.get(WrittenMakimono.WATERLOGGED)), 3); }
						...
						case WEST :
							...
							if (northstate.getMaterial().isReplaceable()) {
								this.writebook(worldIn, pos, playerIn, hand);
								worldIn.setBlockState(pos.north(), Unit_Blocks.WRITTEN_MAKIMONO.getDefaultState()
										.with(WrittenMakimono.H_FACING, Direction.WEST)
										.with(WrittenMakimono.DOWN, state.get(WrittenMakimono.DOWN))
										.with(WrittenMakimono.WATERLOGGED, state.get(WrittenMakimono.WATERLOGGED)), 3); }
