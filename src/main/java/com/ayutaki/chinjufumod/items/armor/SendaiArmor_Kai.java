...
public class SendaiArmor_Kai extends ArmorItem {

	public SendaiArmor_Kai(IArmorMaterial material, EquipmentSlotType slot, Properties properties) {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entity, int slot, boolean selected) {
		...
		if(entity instanceof PlayerEntity) {
			...
			if(playerIn.getItemStackFromSlot(EquipmentSlotType.FEET).getItem() == Items_Armor.SENDAI_BOOTS_KAI) {
	...
	public <Armor extends BipedModel<?>> Armor getArmorModel(LivingEntity entityLiving, ItemStack itemStack, EquipmentSlotType armorSlot, Armor _default) {
		...
		SendaiModel armorModel = new SendaiModel(1.0F);
		SendaiModel armorModelLegs = new SendaiModel(0.5F);
