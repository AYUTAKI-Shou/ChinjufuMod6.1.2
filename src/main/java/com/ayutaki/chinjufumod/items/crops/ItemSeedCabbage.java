...
public class ItemSeedCabbage extends ItemBlockBace {

	public ItemSeedCabbage() {
		super(Crop_Blocks.CABBAGE);
		setUnlocalizedName("item_seeds_cabbage");
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, "item_seeds_cabbage"));
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (iblockstate.getBlock() == Blocks.FARMLAND && facing == EnumFacing.UP) {
			...
			if (playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn.getBlockState(pos.up()).getMaterial().isReplaceable()) {
				...
				/** 生成するブロック **/
				IBlockState iblockstate1 = Crop_Blocks.CABBAGE.getDefaultState()
