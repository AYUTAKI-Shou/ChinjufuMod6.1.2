...
public class ItemSeedOnion extends ItemBlockBace {

	public ItemSeedOnion() {
		super(Crop_Blocks.ONION);
		setUnlocalizedName("item_seeds_onion");
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, "item_seeds_onion"));
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (iblockstate.getBlock() == Blocks.FARMLAND && facing == EnumFacing.UP) {
			...
			if (playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn.getBlockState(pos.up()).getMaterial().isReplaceable()) {
				...
				/** 生成するブロック **/
				IBlockState iblockstate1 = Crop_Blocks.ONION.getDefaultState()
