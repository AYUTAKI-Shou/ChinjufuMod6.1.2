...
public class ItemKakigouri_Green extends ItemFood {

	public ItemKakigouri_Green(int amount, float saturation, boolean isWolfFood) {
		super(amount, saturation, isWolfFood);
		setCreativeTab(ChinjufuModTabs.SEASONAL);
		setUnlocalizedName(Kakigouri_Green.ID);
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, Kakigouri_Green.ID));
	...
	protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer playerIn) {
		...
		playerIn.addPotionEffect(new PotionEffect(MobEffects.HASTE, 1900, 0));
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!block.isReplaceable(worldIn, pos)) { pos = pos.offset(facing); }
			...
			if (!itemstack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn
					.mayPlace(Dish_Blocks.KAKIGOURI_green, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

				/** 設置するブロックのクラス **/
				IBlockState iblockstate1 = Dish_Blocks.KAKIGOURI_green.getDefaultState()
