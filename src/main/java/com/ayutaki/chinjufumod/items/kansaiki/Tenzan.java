...
public class Tenzan extends BaseKansaiki {

	private final float attackDamage = 7.0F - 1.0F;
	private final float attackSpeed = -2.4F;
	private final Multimap<Attribute, AttributeModifier> defaultModifiers;

	public Tenzan(Item.Properties properties) {
		super(properties);
			...
			if (!playerIn.isUnderWater()) {
				...
				if (mode) {
					...
					if (playerIn.experienceLevel < 12) {
						KK_TenzanEntity kansaiki = new KK_TenzanEntity(playerIn, worldIn, itemstack);
					...
					if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
						KK_TenzanEntity2 kansaiki = new KK_TenzanEntity2(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
						KK_TenzanEntity3 kansaiki = new KK_TenzanEntity3(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 25) {
						KK_TenzanEntity4 kansaiki = new KK_TenzanEntity4(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_kk_tenzan")).withStyle(TextFormatting.DARK_GREEN));
	}
