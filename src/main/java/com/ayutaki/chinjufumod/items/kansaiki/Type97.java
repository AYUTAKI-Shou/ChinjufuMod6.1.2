...
public class Type97 extends BaseKansaiki {
			...
			if (!playerIn.isUnderWater()) {
				...
				if (mode) {
					...
					if (playerIn.experienceLevel < 12) {
						KK_Type97Entity kansaiki = new KK_Type97Entity(playerIn, worldIn, itemstack);
					...
					if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
						KK_Type97Entity2 kansaiki = new KK_Type97Entity2(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
						KK_Type97Entity3 kansaiki = new KK_Type97Entity3(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 25) {
						KK_Type97Entity4 kansaiki = new KK_Type97Entity4(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_kk_type97")).withStyle(TextFormatting.DARK_GREEN));
	}
