...
public class Ryusei extends BaseKansaiki {

	private final float attackDamage = 10.0F - 1.0F;
	private final float attackSpeed = -2.4F;
	private final Multimap<Attribute, AttributeModifier> defaultModifiers;

	public Ryusei(Item.Properties properties) {
		super(properties);
			...
			if (!playerIn.isUnderWater()) {
				...
				if (mode) {
					...
					if (playerIn.experienceLevel < 12) {
						KK_RyuseiEntity kansaiki = new KK_RyuseiEntity(playerIn, worldIn, itemstack);
					...
					if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
						KK_RyuseiEntity2 kansaiki = new KK_RyuseiEntity2(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
						KK_RyuseiEntity3 kansaiki = new KK_RyuseiEntity3(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 25) {
						KK_RyuseiEntity4 kansaiki = new KK_RyuseiEntity4(playerIn, worldIn, itemstack);
						kansaiki.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 0.25F * j, 1.0F);
						worldIn.addFreshEntity(kansaiki); }
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_kk_ryusei")).withStyle(TextFormatting.DARK_GREEN));
	}
