...
public class Item_Type97 extends Item_BaseKansaiki {

	private final float attackDamage = 5.0F - 1.0F;
	private final float attackSpeed = -2.4F;

	public Item_Type97(String unlocalizedName) {
			...
			if (!playerIn.isInsideOfMaterial(Material.WATER)) {
				...
				if (playerIn.capabilities.isCreativeMode) {
					...
					if (playerIn.experienceLevel < 12) {
						Entity_Type97 kansaiki = new Entity_Type97(worldIn, playerIn, itemstack);
					...
					if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
						Entity_Type97_2 kansaiki = new Entity_Type97_2(worldIn, playerIn, itemstack);
						kansaiki.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, 0.25F * j, 1.0F);
						worldIn.spawnEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
						Entity_Type97_3 kansaiki = new Entity_Type97_3(worldIn, playerIn, itemstack);
						kansaiki.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, 0.25F * j, 1.0F);
						worldIn.spawnEntity(kansaiki); }
					
					if (playerIn.experienceLevel >= 25) {
						Entity_Type97_4 kansaiki = new Entity_Type97_4(worldIn, playerIn, itemstack);
						kansaiki.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, 0.25F * j, 1.0F);
						worldIn.spawnEntity(kansaiki); }
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_kk_type97", meta));
	}
}
