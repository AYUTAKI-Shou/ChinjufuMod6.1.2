...
public class ItemGlass_Wine extends ItemBlockBace {

	public ItemGlass_Wine() {
		super(Hakkou_Blocks.WINEGLASS);
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, Glass_Wine.ID));
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!itemstack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn
				.mayPlace(Hakkou_Blocks.WINEGLASS, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

			if (k == 1) {
				/** 設置するブロックのクラス **/
				IBlockState iblockstate1 = Hakkou_Blocks.WINEGLASS.getDefaultState().withProperty(Glass_Wine.STAGE_1_15, Integer.valueOf(1));
				worldIn.setBlockState(pos, iblockstate1, 10); }
			if (k == 2) {
				IBlockState iblockstate1 = Hakkou_Blocks.WINEGLASS.getDefaultState().withProperty(Glass_Wine.STAGE_1_15, Integer.valueOf(4));
				worldIn.setBlockState(pos, iblockstate1, 10); }
			if (k == 3) {
				IBlockState iblockstate1 = Hakkou_Blocks.WINEGLASS.getDefaultState().withProperty(Glass_Wine.STAGE_1_15, Integer.valueOf(7));
				worldIn.setBlockState(pos, iblockstate1, 10); }
			if (k == 4) {
				IBlockState iblockstate1 = Hakkou_Blocks.WINEGLASS.getDefaultState().withProperty(Glass_Wine.STAGE_1_15, Integer.valueOf(10));
				worldIn.setBlockState(pos, iblockstate1, 10); }

			if (k == 5) {
				IBlockState iblockstate1 = Hakkou_Blocks.SAKEGLASS.getDefaultState().withProperty(Glass_Sake.STAGE_1_15, Integer.valueOf(10));
				worldIn.setBlockState(pos, iblockstate1, 10); }
			if (k == 6) {
				IBlockState iblockstate1 = Hakkou_Blocks.SAKEGLASS.getDefaultState().withProperty(Glass_Sake.STAGE_1_15, Integer.valueOf(13));
				worldIn.setBlockState(pos, iblockstate1, 10); }

			worldIn.playSound(null, pos, SoundEvents.BLOCK_STONE_PLACE, SoundCategory.BLOCKS, 0.5F, 0.8F);
			itemstack.shrink(1);
