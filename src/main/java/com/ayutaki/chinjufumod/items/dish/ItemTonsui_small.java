...
public class ItemTonsui_small extends ItemFood {

	public ItemTonsui_small(int amount, float saturation, boolean isWolfFood) {
		super(amount, saturation, isWolfFood);
		setCreativeTab(ChinjufuModTabs.TEATIME);
		setUnlocalizedName(Tonsui_small.ID);
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, Tonsui_small.ID));
	}
	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase entityLiving) {
		...
		if (playerIn == null || !playerIn.capabilities.isCreativeMode) {
			if (stack.isEmpty()) { return new ItemStack(Items_Teatime.Item_DISH, 1, 5); }
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Items_Teatime.Item_DISH, 1, 5))) {
				playerIn.dropItem(new ItemStack(Items_Teatime.Item_DISH, 1, 5), false); }
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!block.isReplaceable(worldIn, pos)) { pos = pos.offset(facing); }
			...
			if (!itemstack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn
					.mayPlace(Dish_Blocks.TONSUITORI, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

				/** 設置するブロックのクラス **/
				IBlockState iblockstate1 = Dish_Blocks.TONSUITORI.getDefaultState()
