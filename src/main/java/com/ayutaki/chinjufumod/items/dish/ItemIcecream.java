...
public class ItemIcecream extends ItemFood {
	...
	public ItemIcecream(int amount, float saturation, boolean isWolfFood) {
		...
		setUnlocalizedName(Icecream.ID);
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, Icecream.ID));
	...
	protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer playerIn) {
		/** ポーションエフェクトの追加 一口100 通常 120 かき氷の倍 **/
		playerIn.addPotionEffect(new PotionEffect(new PotionEffect(MobEffects.LUCK, 3750, 0)));
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!block.isReplaceable(worldIn, pos)) { pos = pos.offset(facing); }
			...
			if (!itemstack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn
					.mayPlace(Dish_Blocks.ICECREAM, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

				/** 設置するブロックのクラス **/
				IBlockState iblockstate1 = Dish_Blocks.ICECREAM.getDefaultState()
