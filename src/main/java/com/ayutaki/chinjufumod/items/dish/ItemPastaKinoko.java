...
public class ItemPastaKinoko extends ItemFood {

	public ItemPastaKinoko(int amount, float saturation, boolean isWolfFood) {
		super(amount, saturation, isWolfFood);
		setCreativeTab(ChinjufuModTabs.TEATIME);
		setUnlocalizedName(PastaKinoko.ID);
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, PastaKinoko.ID));
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!block.isReplaceable(worldIn, pos)) { pos = pos.offset(facing); }
			...
			if (!itemstack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn
					.mayPlace(Dish_Blocks.PASTAKINOKO, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

				/** 設置するブロックのクラス **/
				IBlockState iblockstate1 = Dish_Blocks.PASTAKINOKO.getDefaultState()
