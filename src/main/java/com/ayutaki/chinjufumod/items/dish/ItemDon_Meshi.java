...
public class ItemDon_Meshi extends ItemFood {

	public ItemDon_Meshi(int amount, float saturation, boolean isWolfFood) {
		super(amount, saturation, isWolfFood);
		setCreativeTab(ChinjufuModTabs.TEATIME);
		setUnlocalizedName(Don_Meshi.ID);
		setRegistryName(new ResourceLocation(ChinjufuMod.MOD_ID, Don_Meshi.ID));
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!block.isReplaceable(worldIn, pos)) { pos = pos.offset(facing); }
			...
			if (!itemstack.isEmpty() && facing == EnumFacing.UP && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn
					.mayPlace(Dish_Blocks.DONBURI_MESHI, pos, false, facing, (Entity)null) && (playerIn.isSneaking() || playerIn.isRiding())) {

				/** 設置するブロックのクラス **/
				IBlockState iblockstate1 = Dish_Blocks.DONBURI_MESHI.getDefaultState()
