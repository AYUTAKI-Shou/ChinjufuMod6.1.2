...
public class ItemRensouHou127 extends ItemBow {

	public ItemRensouHou127() {
		this.maxStackSize = 1;
		setMaxDamage(1000);
		...
		this.addPropertyOverride(new ResourceLocation("pull"), new IItemPropertyGetter() {
			...
			public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) {
				...
				if (entityIn == null) {
					...
					return entityIn.getActiveItemStack().getItem() != Items_Weapon.RENSOUHOU_127 ? 0.0F : (float)(stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) / 20.0F;
				}
			}
		});
				...
				if ((double)f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						...
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage()); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + (double)j * 0.5D); }
						}
						...
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 1.0D); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 1.5D); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + (double)j * 0.5D + 1.5D); }
						}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_rensouhou127", meta));
	}
}
