...
public class ItemRensouHou203 extends ItemBow {
		...
		this.addPropertyOverride(new ResourceLocation("pull"), new IItemPropertyGetter() {
			...
			public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) {
				...
				if (entityIn == null) {
					...
					return entityIn.getActiveItemStack().getItem() != Items_Weapon.RENSOUHOU_203 ? 0.0F : (float)(stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) / 20.0F;
				}
			}
		});
				...
				if ((double)f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						...
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + 0.5D); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + 1.0D); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + 1.5D); }
							if (j > 0) { entityarrow.setDamage(entityarrow.getDamage() + 3.0D + (double)j * 0.5D + 1.5D); }
						}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_rensouhou203", meta));
	}
}
