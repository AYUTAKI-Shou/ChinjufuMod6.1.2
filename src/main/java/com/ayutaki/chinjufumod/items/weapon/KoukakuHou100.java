...
public class KoukakuHou100 extends BowItem {

	public KoukakuHou100(Item.Properties builder) {
		super(builder);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!itemstack.isEmpty() || mode) {
				...
				if (!((double)f < 0.1D)) {
					...
					boolean mode1 = playerIn.abilities.instabuild || (itemstack.getItem() instanceof Ammo_Small && ((Ammo_Small)itemstack.getItem()).isInfinite(itemstack, stack, playerIn));
					...
					if (!worldIn.isClientSide) {
						...
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + 0.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + 1.0D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + 1.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 0.5D + (double)j * 0.5D + 1.5D); }
						}
					...
					if (!worldIn.isClientSide) {
						if (!playerIn.abilities.instabuild) {
							playerIn.drop(new ItemStack(Items_Weapon.CARTRIDGE_S), false);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_koukakuhou100")).withStyle(TextFormatting.DARK_GREEN));
	}
