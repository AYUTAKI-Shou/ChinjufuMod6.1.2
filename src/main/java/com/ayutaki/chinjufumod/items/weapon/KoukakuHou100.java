...
public class KoukakuHou100 extends BowItem {
	...
	public KoukakuHou100(Item.Properties builder) {
		...
		if (entity == null) {
			...
			return !(entity.getActiveItemStack().getItem() instanceof KoukakuHou100) ? 0.0F : (float)(stack.getUseDuration() - entity.getItemInUseCount()) / 20.0F;
		} } );
			...
			if (!itemstack.isEmpty() || mode) {
				...
				if (!((double)f < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						...
						/* ダメージは弓矢 (3.0-2.0)÷2 (艦これ数値－弾薬値)÷２*/
						int j = EnchantmentHelper.getEnchantmentLevel(Enchantments.POWER, stack);
						
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + 0.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + 1.0D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + 1.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 0.5D + (double)j * 0.5D + 1.5D); }
						}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_koukakuhou100")).applyTextStyle(TextFormatting.DARK_GREEN));
	}
