...
public class RensouHou380 extends BowItem {

	public RensouHou380(Item.Properties builder) {
		super(builder);
	}
			...
			if (!itemstack.isEmpty() || mode) {
				...
				if (!((double)f < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						...
						/* 基本値にレベルに応じて加算 標準で 標準で POWER +(17.0-2.0)÷2 */
						int j = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + 0.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + 1.0D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + 1.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 7.5D + (double)j * 0.5D + 1.5D); }
						}
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_rensouhou380")).withStyle(TextFormatting.DARK_GREEN));
	}
