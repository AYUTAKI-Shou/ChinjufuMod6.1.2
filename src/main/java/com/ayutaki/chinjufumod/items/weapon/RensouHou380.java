...
public class RensouHou380 extends BowItem {
	...
	public RensouHou380(Item.Properties builder) {
		...
		if (entity == null) {
			...
			return !(entity.getActiveItemStack().getItem() instanceof RensouHou380) ? 0.0F : (float)(stack.getUseDuration() - entity.getItemInUseCount()) / 20.0F;
		} } );
				...
				if (!((double)f < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						...
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + 0.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + 1.0D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + 1.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 7.5D + (double)j * 0.5D + 1.5D); }
						}
