...
public class RensouHou203SKC34 extends BowItem {

	public RensouHou203SKC34(Item.Properties builder) {
		super(builder);
		...
		if (entity == null) {
			...
			return !(entity.getActiveItemStack().getItem() instanceof RensouHou203SKC34) ? 0.0F : (float)(stack.getUseDuration() - entity.getItemInUseCount()) / 20.0F;
		} } );
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!itemstack.isEmpty() || mode) {
				if (itemstack.isEmpty()) {
					itemstack = new ItemStack(Items_Weapon.AMMUNITION_M);
				}
				...
				if (!((double)f < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						...
						/* 標準で POWER (10.0-2.0)÷2 */
						int j = EnchantmentHelper.getEnchantmentLevel(Enchantments.POWER, stack);

						if (playerIn.experienceLevel < 12) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + 0.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + 1.0D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + 1.5D); }
							if (j > 0) { abstractarrowentity.setDamage(abstractarrowentity.getDamage() + 4.0D + (double)j * 0.5D + 1.5D); }
						}
	...
	/* 使用する弾 */
	public static final Predicate<ItemStack> AMMOM = (itemstack) -> {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_rensouhou203_skc34")).applyTextStyle(TextFormatting.DARK_GREEN));
	}
