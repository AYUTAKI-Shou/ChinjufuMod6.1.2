...
public class RensouHou203SKC34 extends BowItem {
			...
			if (!itemstack.isEmpty() || mode) {
				...
				if (!((double)f < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						...
						/* 基本値にレベルに応じて加算 標準で POWER (10.0-2.0)÷2 */
						int j = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.POWER_ARROWS, stack);
						if (playerIn.experienceLevel < 12) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + (double)j * 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 12 && playerIn.experienceLevel < 19) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + 0.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + (double)j * 0.5D + 0.5D); }
						}
						
						if (playerIn.experienceLevel >= 19 && playerIn.experienceLevel < 25) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + 1.0D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + (double)j * 0.5D + 1.0D); }
						}
						
						if (playerIn.experienceLevel >= 25) {
							if (j == 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + 1.5D); }
							if (j > 0) { abstractarrowentity.setBaseDamage(abstractarrowentity.getBaseDamage() + 4.0D + (double)j * 0.5D + 1.5D); }
						}
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag tipFlag) {
		...
		tooltip.add((new TranslationTextComponent("tips.item_rensouhou203_skc34")).withStyle(TextFormatting.DARK_GREEN));
	}
