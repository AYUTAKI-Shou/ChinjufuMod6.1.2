...
public class ItemHake_Red extends Base_ItemHake {
	...
	public ItemHake_Red(String unlocalizedName) {
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (blockIn instanceof BaseFuton) {
			if (blockIn != Futon_Blocks.FUTON_red) {
				CMEvents.soundPaint(worldIn, playerIn, pos);

				worldIn.setBlockState(pos, Futon_Blocks.FUTON_red.getDefaultState()
		...
		if (blockIn instanceof Tatami_H) {
			if (blockIn != JPDeco_Blocks.TATAMI_H_red && blockIn != JPDeco_Blocks.TATAMI_H) {
				CMEvents.soundPaint(worldIn, playerIn, pos);

				worldIn.setBlockState(pos, JPDeco_Blocks.TATAMI_H_red.getDefaultState()
		...
		if (blockIn instanceof Tatami_HY) {
			if (blockIn != JPDeco_Blocks.TATAMI_HY_red && blockIn != JPDeco_Blocks.TATAMI_HY) {
				CMEvents.soundPaint(worldIn, playerIn, pos);

				worldIn.setBlockState(pos, JPDeco_Blocks.TATAMI_HY_red.getDefaultState()
						.withProperty(BaseFacingSlabW.H_FACING, stateIn.getValue(BaseFacingSlabW.H_FACING))
						.withProperty(BaseFacingSlabW.DOUBLE, stateIn.getValue(BaseFacingSlabW.DOUBLE))
						.withProperty(BaseFacingSlabW.HALF, stateIn.getValue(BaseFacingSlabW.HALF)), 11);
