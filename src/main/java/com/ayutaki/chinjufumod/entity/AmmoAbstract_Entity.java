...
public abstract class AmmoAbstract_Entity extends Entity implements IProjectile {

	private static final DataParameter<Byte> CRITICAL = EntityDataManager.createKey(AmmoAbstract_Entity.class, DataSerializers.BYTE);
	protected static final DataParameter<Optional<UUID>> field_212362_a = EntityDataManager.createKey(AmmoAbstract_Entity.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	private static final DataParameter<Byte> PIERCE_LEVEL = EntityDataManager.createKey(AmmoAbstract_Entity.class, DataSerializers.BYTE);
	@Nullable
	private BlockState inBlockState;
	protected boolean inGround;
	protected int timeInGround;
	public AmmoAbstract_Entity.PickupStatus pickupStatus = AmmoAbstract_Entity.PickupStatus.DISALLOWED;
	...
	protected AmmoAbstract_Entity(EntityType<? extends AmmoAbstract_Entity> type, LivingEntity shooter, World worldIn) {
		...
		if (shooter instanceof PlayerEntity) {
			this.pickupStatus = AmmoAbstract_Entity.PickupStatus.ALLOWED;
		}
	}
	...
	protected void onEntityHit(EntityRayTraceResult p_213868_1_) {
		...
		if (entity.attackEntityFrom(damagesource, (float)i)) {
			...
			if (!this.world.isRemote && this.getMotion().lengthSquared() < 1.0E-7D) {
				if (this.pickupStatus == AmmoAbstract_Entity.PickupStatus.ALLOWED) {
	...
	public void readAdditional(CompoundNBT compound) {
		...
		if (compound.contains("pickup", 99)) {
			this.pickupStatus = AmmoAbstract_Entity.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		}

		else if (compound.contains("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? AmmoAbstract_Entity.PickupStatus.ALLOWED : AmmoAbstract_Entity.PickupStatus.DISALLOWED;
		}
	...
	public void setShooter(@Nullable Entity entityIn) {
		...
		if (entityIn instanceof PlayerEntity) {
			this.pickupStatus = ((PlayerEntity)entityIn).abilities.isCreativeMode ? AmmoAbstract_Entity.PickupStatus.CREATIVE_ONLY : AmmoAbstract_Entity.PickupStatus.ALLOWED;
		}
	}
	...
	public void onCollideWithPlayer(PlayerEntity entityIn) {
		if (!this.world.isRemote && (this.inGround || this.getNoClip()) && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == AmmoAbstract_Entity.PickupStatus.ALLOWED || this.pickupStatus == AmmoAbstract_Entity.PickupStatus.CREATIVE_ONLY && entityIn.abilities.isCreativeMode || this.getNoClip() && this.getShooter().getUniqueID() == entityIn.getUniqueID();
			if (this.pickupStatus == AmmoAbstract_Entity.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getArrowStack())) {
				flag = false;
			}
	...
	public static enum PickupStatus {
		...
		public static AmmoAbstract_Entity.PickupStatus getByOrdinal(int ordinal) {
