...
public abstract class Base_EntityAmmo extends Entity implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(Base_EntityAmmo.class, DataSerializers.BYTE);
	protected int xTile;
	protected int yTile;
	protected int zTile;
	...
	public Base_EntityAmmo.PickupStatus pickupStatus;
	...
	public Base_EntityAmmo(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = Base_EntityAmmo.PickupStatus.DISALLOWED;
		
		/** 工廠指示書を難しくした分の加算 + 1.0 して 3.0D **/
		this.damage = 3.0D;
		this.setSize(0.5F, 0.5F);
	}

	public Base_EntityAmmo(World worldIn, double x, double y, double z) {
		this(worldIn);
		this.setPosition(x, y, z);
	}

	public Base_EntityAmmo(World worldIn, EntityLivingBase shooter) {
		this(worldIn, shooter.posX, shooter.posY + (double)shooter.getEyeHeight() - 0.10000000149011612D, shooter.posZ);
		this.shootingEntity = shooter;

		if (shooter instanceof EntityPlayer) {
			this.pickupStatus = Base_EntityAmmo.PickupStatus.ALLOWED;
		}
	}
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float)i)) {
				...
				if (!this.world.isRemote && this.motionX * this.motionX + this.motionY * this.motionY + this.motionZ * this.motionZ < 0.0010000000474974513D) {
					...
					if (this.pickupStatus == Base_EntityAmmo.PickupStatus.ALLOWED) {
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = Base_EntityAmmo.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		}

		else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? Base_EntityAmmo.PickupStatus.ALLOWED : Base_EntityAmmo.PickupStatus.DISALLOWED;
		}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		...
		if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == Base_EntityAmmo.PickupStatus.ALLOWED || this.pickupStatus == Base_EntityAmmo.PickupStatus.CREATIVE_ONLY && entityIn.capabilities.isCreativeMode;

			if (this.pickupStatus == Base_EntityAmmo.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getArrowStack())) {
				flag = false;
			}
	...
	public static enum PickupStatus {
		...
		public static Base_EntityAmmo.PickupStatus getByOrdinal(int ordinal) {
