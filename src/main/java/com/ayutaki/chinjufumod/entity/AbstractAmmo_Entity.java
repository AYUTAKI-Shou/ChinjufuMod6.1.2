...
public abstract class AbstractAmmo_Entity extends AbstractArrowEntity {

	private static final DataParameter<Byte> ID_FLAGS = EntityDataManager.defineId(AbstractAmmo_Entity.class, DataSerializers.BYTE);
	private static final DataParameter<Byte> PIERCE_LEVEL = EntityDataManager.defineId(AbstractAmmo_Entity.class, DataSerializers.BYTE);
	@Nullable
	private BlockState lastState;
	protected boolean inGround;
	protected int inGroundTime;
	public AbstractAmmo_Entity.PickupStatus pickup = AbstractAmmo_Entity.PickupStatus.DISALLOWED;
	public int shakeTime;
	private int life;
	/** 工廠指示書を難しくした分の加算 + 1.0して3.0D **/
	...
	protected AbstractAmmo_Entity(EntityType<? extends AbstractArrowEntity> type, World worldIn) {
		super(type, worldIn);
	}

	protected AbstractAmmo_Entity(EntityType<? extends AbstractArrowEntity> type, double x, double y, double z, World worldIn) {
		this(type, worldIn);
		this.setPos(x, y, z);
	}

	protected AbstractAmmo_Entity(EntityType<? extends AbstractArrowEntity> type, LivingEntity shooter, World worldIn) {
		this(type, shooter.getX(), shooter.getEyeY() - (double)0.1F, shooter.getZ(), worldIn);
		this.setOwner(shooter);
		if (shooter instanceof PlayerEntity) {
			this.pickup = AbstractAmmo_Entity.PickupStatus.ALLOWED;
		}
	}
	...
	protected void onHitEntity(EntityRayTraceResult result) {
		...
		if (entity.hurt(damagesource, (float)i)) {
			...
			if (!this.level.isClientSide && this.getDeltaMovement().lengthSqr() < 1.0E-7D) {
				if (this.pickup == AbstractAmmo_Entity.PickupStatus.ALLOWED) {
	...
	protected void onHitBlock(BlockRayTraceResult result) {
		...
		this.setSoundEvent(SoundEvents_CM.AM_IMPACT);
	...
	public void readAdditionalSaveData(CompoundNBT compound) {
		...
		if (compound.contains("pickup", 99)) {
			this.pickup = AbstractAmmo_Entity.PickupStatus.byOrdinal(compound.getByte("pickup"));
		} else if (compound.contains("player", 99)) {
			this.pickup = compound.getBoolean("player") ? AbstractAmmo_Entity.PickupStatus.ALLOWED : AbstractAmmo_Entity.PickupStatus.DISALLOWED;
		}
	...
	public void setOwner(@Nullable Entity entity) {
		...
		if (entity instanceof PlayerEntity) {
			this.pickup = ((PlayerEntity)entity).abilities.instabuild ? AbstractAmmo_Entity.PickupStatus.CREATIVE_ONLY : AbstractAmmo_Entity.PickupStatus.ALLOWED;
		}
	}
	...
	public void playerTouch(PlayerEntity entity) {
		if (!this.level.isClientSide && (this.inGround || this.isNoPhysics()) && this.shakeTime <= 0) {
			boolean flag = this.pickup == AbstractAmmo_Entity.PickupStatus.ALLOWED || this.pickup == AbstractAmmo_Entity.PickupStatus.CREATIVE_ONLY && entity.abilities.instabuild || this.isNoPhysics() && this.getOwner().getUUID() == entity.getUUID();
			if (this.pickup == AbstractAmmo_Entity.PickupStatus.ALLOWED && !entity.inventory.add(this.getPickupItem())) {
				flag = false;
			}
	...
	public static enum PickupStatus {
		...
		public static AbstractAmmo_Entity.PickupStatus byOrdinal(int value) {
