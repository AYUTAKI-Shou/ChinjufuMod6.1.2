...
/* エンティティなどで呼び出せば使える */
public class GRExplosion extends Explosion {
	...
	public GRExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, List<BlockPos> affectedPositions) {
		this(worldIn, entityIn, x, y, z, size, false, true, affectedPositions);
	}

	@SideOnly(Side.CLIENT)
	public GRExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size,
	...
	public GRExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean flaming, boolean damagesTerrain) {
					...
					if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
						...
						float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
							...
							if (iblockstate.getMaterial() != Material.AIR) {
								...
								float f2 = this.exploder != null ? this.exploder.getExplosionResistance(this, this.world, blockpos, iblockstate) : iblockstate
										.getBlock().getExplosionResistance(world, blockpos, (Entity)null, this);
								f -= (f2 + 0.3F) * 0.3F;
							}
			...
			if (!entity.isImmuneToExplosions()) {
				...
				if (d12 <= 1.0D) {
					...
					if (d13 != 0.0D) {
						...
						/**+ 1.0D) → + 10.0D) **/
						entity.attackEntityFrom(DamageSource.causeExplosionDamage(this), (float)((int)((d10 * d10 + d10) / 2.0D * 7.0D * (double)f3 + 30.0D)));
						double d11 = d10;
						...
						if (entity instanceof EntityPlayer) {
							...
							if (!playerIn.isSpectator() && (!playerIn.isCreative() || !playerIn.capabilities.isFlying)) {
								...
								this.playerKnockbackMap.put(playerIn, new Vec3d(d5 * d10, d7 * d10, d9 * d10));
							}
						}
	...
	public void doExplosionB(boolean spawnParticles) {
		...
		/* 効果音 爆発の召喚側で設定させるため、コメントアウトで無音にする
		...
		/** 爆破サイズが2以上のときはHUGE、2より小さいときはLARGE **/
		...
		if (this.causesFire) {
			...
			for (BlockPos blockpos1 : this.affectedBlockPositions) {
				...
				if (this.world.getBlockState(blockpos1).getMaterial() == Material.AIR && this.world
